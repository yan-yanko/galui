 ...
 *   5. Extracts page data (headings, CTAs, forms, schema.org, text) and pushes to Galui
 *   6. Logs AI agent traffic for analytics
 *
 * Usage:
 *   <script src="https://api.galui.com/galui.js?key=YOUR_KEY" async></script>
 *
...
(function (window, document) {
  ...
  if (!TENANT_KEY) {
    console.warn('[galui] No API key provided. Add ?key=YOUR_KEY to script src.');
    return;
  }
  ...
  //   - llms.txt (Galui-hosted, always available)
  //   - ai-plugin.json (Galui-hosted, always available)
  ...
  function _injectDiscoveryLinks() {
    ...
    if (!document.querySelector('link[href="/llms.txt"]')) {
      ...
      llmsNative.setAttribute('data-galui', 'llms');
      head.appendChild(llmsNative);
    }

    // ── ai-plugin.json (Galui hosted) ────────────────────────────────────────
    ...
    if (!document.querySelector('link[href="/.well-known/ai-plugin.json"]')) {
      ...
      pluginNative.setAttribute('data-galui', 'ai-plugin');
    ...
    // ── Galui verification tag (proof of install, read by /registry/:domain) ─
    if (!document.querySelector('meta[name="galui-verified"]')) {
      var verMeta = document.createElement('meta');
      verMeta.name    = 'galui-verified';
  ...
  function _injectSchemaIfMissing() {
    ...
    var description = _extractMetaDescription() || 'Website powered by Galui AI Readability.';
    ...
    if (schema) {
      ...
      scriptEl.setAttribute('data-galui', 'schema');
  ...
  function _extractSchemaOrg() {
    ...
    for (var i = 0; i < scripts.length; i++) {
      ...
      if (scripts[i].getAttribute('data-galui') === 'schema') continue;
      try {
  ...
  // ── 8. Main init ───────────────────────────────────────────────────────────
  function _init() {
    log('Initializing galui v3.0.0 for domain:', domain);
